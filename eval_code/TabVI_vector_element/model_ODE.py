...
class CT_Transformer(nn.Module):
    def __init__(self, time_emb_kind, omega_0, enc_ctsa, dec_ctsa, enc_in, dec_in, c_out, seq_len, label_len, out_len, 
                factor = 5, d_model = 512, n_heads = 8, e_layers = 3, d_layers = 2, d_ff = 512, 
            ...
            [
                DecoderLayer(
                    AttentionLayer(CTAttn(True, factor, attention_dropout=dropout, output_attention=False,
                                               ...
                                               odeint_atol = odeint_atol, 
                                               method = method), 
                                d_model, n_heads) if dec_ctsa else AttentionLayer(Attn(True, factor, attention_dropout=dropout, output_attention=False), 
                                            d_model, n_heads),
                    AttentionLayer(FullAttention(False, factor, attention_dropout=dropout, output_attention=False), 
                                d_model, n_heads),
                    d_model,
