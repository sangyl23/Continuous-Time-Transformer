...
class Dataloader():
    ...
    def __init__(self, path = '', batch_size = 64, device = 'cpu', 
        ...
        self.his_timeslot_idx = self.set_his_timeslot_idx(his_len)
    ...
    def reset(self):
        ...
        self.buffer_random_timeslot = np.zeros((0, self.his_len))
    
    def channelnorm(H):
        H = H / np.sqrt(np.mean(np.abs(H)**2))
        return H
    
    def set_his_timeslot_idx(self, his_len):
        if his_len == 29:
            idx = np.array([0, 2, 4, 6])
            tmp = idx
            for i in range(6):
                tmp = tmp + 7 
                idx = np.concatenate((idx, tmp), axis = 0)
            idx = np.concatenate((idx, np.array([49])), axis = 0)
        elif his_len == 36:
    ...
    def load(self, file):
        ...
        random_timeslot = data['his_timeslot'] - 1 # (b, 50)
        rows = np.arange(random_timeslot.shape[0])[:, None]  # shape: (256, 1)
        random_timeslot = random_timeslot[rows, self.his_timeslot_idx]  # shape: (256, his_len)
        random_timeslot = random_timeslot.astype(int)
        random_timeslot = random_timeslot[:, : self.his_len]
        random_timeslot_tmp = np.expand_dims(random_timeslot, axis = 1) # (b, 1, his_len)
        random_timeslot_tmp = np.broadcast_to(random_timeslot_tmp, (256, self.M, self.his_len)) # (b, M, his_len)
