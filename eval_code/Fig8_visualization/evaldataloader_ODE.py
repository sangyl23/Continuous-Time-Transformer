...
class Dataloader():
    ...
    def __init__(self, path = '', batch_size = 64, device = 'cpu', 
                 ...
                 ifintepolate_pilot = 'No', intepolate_points = np.array([])):
        ...
        self.intepolate_points = intepolate_points * 10
    ...
    def getfileidx_py(self):
        numbers = []
        for p in self.files:
            match = re.search(r'dataset_(\d+)\.mat', p)
            if match:
    ...
    def load(self, file):
        ...
        #############################################################
        pre_step = int(400 / self.pre_len)
        tmp = np.arange(2801, 3201, step = pre_step)
        pre_timeslot = np.tile(tmp, (256, 1))
        #############################################################
        
        pre_timeslot_tmp = np.expand_dims(pre_timeslot, axis = 1) # (b, 1, pre_len)
        
        # print(pre_timeslot_tmp.shape)
        # print(self.pre_len)
        ...
        if self.ifintepolate_pilot == False:
            ...
            his_channel = np.stack((train_r[:, :, 0 : 29], train_i[:, :, 0 : 29]), axis = 1) # (b, 2, M, his_len)
        else:
            raise NotImplementedError
        
        pre_channel = np.stack((train_r_gt[:, :, 29 : 429 : pre_step], train_i_gt[:, :, 29 : 429 : pre_step]), axis = 1)
        ...
        return his_channel, pre_channel, np.concatenate((self.intepolate_points[-1].astype(int) * np.ones((256, 1)), pre_timeslot), axis = 1) # (b, pre_len + 1) 
    ...
    def next_batch(self):
        ...
        while self.buffer_train.shape[0] < self.batch_size:
            ...
            if len(self.unvisited_files) == 0:
                ...
                break
            his_channel, pre_channel, pre_timeslot = self.load(self.unvisited_files.pop(0))
        ...
        if done == False:
            try:
                ...
                print("There exist different elements among different rows:", e)
                sys.exit(1)
